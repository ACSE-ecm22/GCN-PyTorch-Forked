...
def save_generative_fts(aggregation_z):
    ...
    pickle.dump(save_fts, open(os.path.join("../", 'features', method_name,
...
if __name__ == "__main__":
    ...
    data = NodeClsData(norm_adj=False)
    adj, true_features = data.adj, data.features
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.dataset in ['cora', 'citeseer', 'steam']:
            ...
            if eva_values_list[-1] > best_recall:
                torch.save(model.state_dict(), os.path.join("../", 'output', method_name,
...
model.load_state_dict(torch.load(os.path.join("../", 'output', method_name,
                                                        'best_gene_{}_{}.pkl'.format(args.dataset, train_fts_ratio))))
...
pickle.dump(known_node_idx, open(os.path.join("../", 'features', method_name,
                                            'known_idx_train_ratio_{}_{}.pkl'.format(args.dataset, train_fts_ratio)), 'wb'))
pickle.dump(unknown_node_idx, open(os.path.join("../", 'features', method_name,
                                            'unknown_idx_train_ratio_{}_{}.pkl'.format(args.dataset, train_fts_ratio)), 'wb'))
pickle.dump(true_features, open(os.path.join("../", 'features', method_name,
